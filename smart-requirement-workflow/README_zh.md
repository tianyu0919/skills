...
- **已有 Spec 检查**（阶段三）：新建 spec 前先扫描 `sdd/specs/` 是否存在可复用或可续接的工作，避免重复劳动。
- **五段式文档框架**（阶段三）：确认无误后，自动生成 5 份核心文档：
  - `proposal.md` — 背景痛点、价值主张、备选方案、成功指标
  - `design.md` — 技术架构、数据模型、接口定义、数据流、错误处理策略
  - `spec.md` — YAML Frontmatter、范围、**SHALL/Scenario** 格式的功能需求
  - `tasks.md` — 有序任务看板，含**显式依赖声明**和并行标记
  - `checklist.md` — 独立验收清单（每项均可客观验证）
...
### 1. 新增需求
当新增需求时，会在 `sdd/specs/<requirement-name>/` 下创建目录，使用**动词引导的命名**（如 `add-user-login`、`migrate-auth-to-jwt`），包含以下五个核心文件：
